...
def get_wire_info(wire_dir, img):
    '''
...
def combine(main_dir, wire_dir, orig_device_dir, orig_wire_img_lst, background_img_lst, target_dir, num_of_images, categories, class_dict):
    ...
    for i in tqdm(range(num_of_images)):
        ...
        bg_arr = cv2.imread(bg_path).astype('uint8')
...
def main():
    ...
    date = '20221006'
    username = 'wall-e'
    datasets_dir = '/home/' + username + '/astrobee/datasets'
    main_dir = datasets_dir + f'/{date}'
    all_wire_dir = '/home/' + username + '/astrobee/datasets/single_wire'
    num_of_train_images = 4717
    num_of_val_easy_images = 1000
    ...
    try: shutil.copy('/home/' + username + '/astrobee/datasets/export.json', export_json)
    except: print(f'export.json already exists in {main_dir}')
    
    dirs = {original: f'/home/{username}/astrobee/datasets/{date}/original',
            val_easy_original: f'/home/{username}/astrobee/datasets/{date}/val_easy_original',
            val: f'/home/{username}/astrobee/datasets/{date}/val',
            val_hard: f'/home/{username}/astrobee/datasets/{date}/val_hard'}
    ...
    all_wire_lst = os.listdir(all_wire_dir)
    train_wire_lst = []
    val_easy_wire_lst = []
    for i in range (0, len(all_wire_lst)):
        rand = random.random()
        if 0 <= rand < 0.8:
            train_wire_lst.append(all_wire_lst[i])
        else:
            val_easy_wire_lst.append(all_wire_lst[i])
    # print(len(val_easy_wire_lst))
            
    all_backgrounds_dir = main_dir + '/backgrounds'
    all_backgrounds_lst = os.listdir(all_backgrounds_dir)
    train_backgrounds_lst = []
    val_easy_backgrounds_lst = []
    for i in range (0, len(all_backgrounds_lst)):
        rand = random.random()
        if 0 <= rand < 0.8:
            train_backgrounds_lst.append(all_backgrounds_lst[i])
        else:
            val_easy_backgrounds_lst.append(all_backgrounds_lst[i])
       
    # generate train
    print("Generating 'train' data")
    coco_new_obj_sem, coco_new_obj_seg_sem = combine(main_dir = main_dir, wire_dir=all_wire_dir, orig_device_dir = original, orig_wire_img_lst = train_wire_lst, background_img_lst = train_backgrounds_lst, target_dir = train, num_of_images = num_of_train_images, categories = categories, class_dict=class_dict)
    ...
    coco_new_obj_sem, coco_new_obj_seg_sem = combine(main_dir = main_dir, wire_dir=all_wire_dir, orig_device_dir = val_easy_original, orig_wire_img_lst = val_easy_wire_lst, background_img_lst = val_easy_backgrounds_lst, target_dir = val_easy, num_of_images = num_of_val_easy_images, categories = categories, class_dict=class_dict)
