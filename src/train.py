...
class Train:
    ...
    def train(self):
        ...
        if self.train_detectron2:
            shutil.move(
                self.events_dir + f"{self.name}_{self.model}_{self.pr}/model_final.pth",
                f"./train/trained-models/{self.name}_{self.model}_{self.pr}.pth",
            )
            print(
                f"Training complete! Model saved in ./train/trained-models/{self.name}_{self.model}_{self.pr}.pth!"
            )
