...
class Cocopen:
    ...
    def scale_image(self, img_arr, masks, scale):
        ...
        new_img_arr = cv2.resize(img_arr, (int(self.width * scale), int(self.height * scale)))
        new_masks = []
        for mask in masks:
            new_masks.append(cv2.resize(mask, (int(self.width * scale), int(self.height * scale))))
        ...
        if int(self.width * scale) < self.width or int(self.height * scale) < self.height:
    ...
    def random_operations(self, img, angle, flip_h, flip_v):
        ...
        rot_mat = cv2.getRotationMatrix2D(
            image_center, angle, self.width / (self.height * self.width / np.sqrt(self.width**2 + self.height**2))
        )
