...
def download_single_wire_image_from_azure(img, single_wire_container_client):
    '''
    Downloads single_wire images from Azure blob storage
    '''
    # Get blob and read image data into memory cache
    blob = single_wire_container_client.get_blob_client(img)
...
def download_background_image_from_azure(bg, background_container_client):
    '''
    Downloads background images from Azure blob storage
    '''
    # Get blob and read image data into memory cache
    blob = background_container_client.get_blob_client(bg)
    img_data = blob.download_blob().readall()
    # Convert image data to numpy array
    img_array = np.asarray(bytearray(img_data), dtype=np.uint8)
    # Decode image
...
def get_wire_info(wire_dir, img, single_wire_container_client):
    ...
    '''
    # Get wire mask
    src = download_single_wire_image_from_azure(img=img, single_wire_container_client=single_wire_container_client)
...
def combine(main_dir, wire_dir, orig_device_dir, orig_wire_img_lst, background_img_lst, target_dir, num_of_images, categories, class_dict, single_wire_container_client, background_container_client):
    ...
    for i in tqdm(range(num_of_images)):
        ...
        src = download_background_image_from_azure(bg=bg, background_container_client=background_container_client)
